...
with secondSection:
    ...
    with upper:
        ...
        with left:
            st.write("Configure and compare currencies here by selecting the desired currency and time period.")
            [firstColumn, secondColumn, thirdColumn] = st.columns(3)

            with firstColumn:
                #selectedCurrency = st.selectbox("Select Currency", currencyTypes)
                st.write("This is the left side upper")
                
            with secondColumn:
                timePeriod = st.selectbox("Select Time Period", ["1 year", "5 years", "10 years", "20 years"])
